...
def get_blender_connection():
    ...
    if _blender_connection is None:
        _blender_connection = BlenderConnection(host="localhost", port=9877)
...
def add_geometry_nodes(ctx: Context, object_name: str, modifier_name: str = None) -> str:
    ...
    - modifier_name: Optional name for the modifier (defaults to "{object_name}_GeometryNodes")
    """
    try:
        blender = get_blender_connection()
        ...
        if "error" in result:
            return f"Error adding Geometry Nodes: {result['error']}"
        
        return f"Added Geometry Nodes modifier '{result['modifier_name']}' to {object_name}"
    except Exception as e:
        logger.error(f"Error adding Geometry Nodes: {str(e)}")
...
def modify_geometry_nodes(ctx: Context, object_name: str, modifier_name: str, operations: list) -> str:
    ...
    Parameters:
    - object_name: Name of the object with the Geometry Nodes modifier
    - modifier_name: Name of the Geometry Nodes modifier
    - operations: List of operations to perform on the node group
    
    Operations can include:
    - Adding nodes: {"type": "add_node", "params": {"node_type": "GeometryNodeMeshCube", "location": [0, 0]}}
    - Creating links: {"type": "add_link", "params": {"from_node": "Node1", "from_socket": "Geometry", "to_node": "Node2", "to_socket": "Geometry"}}
    - Setting properties: {"type": "set_node_property", "params": {"node_name": "Node1", "property_name": "inputs.Size", "property_value": 2.0}}
    """
    try:
        blender = get_blender_connection()
        ...
        if "error" in result:
            return f"Error modifying Geometry Nodes: {result['error']}"
        
        return f"Modified Geometry Nodes setup '{modifier_name}' on {object_name}"
    except Exception as e:
        logger.error(f"Error modifying Geometry Nodes: {str(e)}")
...
def get_geometry_nodes_info(ctx: Context, object_name: str, modifier_name: str = None) -> str:
    ...
    - modifier_name: Optional name of a specific Geometry Nodes modifier to get detailed info about
    """
    try:
        blender = get_blender_connection()
        ...
        if "error" in result:
            return f"Error getting Geometry Nodes info: {result['error']}"
        
        return json.dumps(result, indent=2)
    except Exception as e:
        logger.error(f"Error getting Geometry Nodes info: {str(e)}")
...
def remove_geometry_nodes(ctx: Context, object_name: str, modifier_name: str) -> str:
    ...
    Parameters:
    - object_name: Name of the object with the Geometry Nodes modifier
    - modifier_name: Name of the Geometry Nodes modifier to remove
    """
    try:
        blender = get_blender_connection()
        ...
        if "error" in result:
            return f"Error removing Geometry Nodes: {result['error']}"
        
        return f"Removed Geometry Nodes modifier '{modifier_name}' from {object_name}"
    except Exception as e:
        logger.error(f"Error removing Geometry Nodes: {str(e)}")
        return f"Error removing Geometry Nodes: {str(e)}"
...
def asset_creation_strategy() -> str:
    ...
    1. First use the following tools to verify if the following integrations are enabled:
        ...
        3. Geometry Nodes (v2.1+)
            ...
            - Adding randomized variations to meshes
            - Building parametric shapes
